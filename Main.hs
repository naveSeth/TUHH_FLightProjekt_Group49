...
move (xdim, ydim) "\"s\"" (x, y) = (x, (y + 1) `mod` ydim)
move (xdim, ydim) "\"w\"" (x, y) = (x, (y - 1) `mod` ydim)
move (xdim, ydim) "\"a\"" (x, y) = ((x - 1) `mod` xdim, y)
move (xdim, ydim) "\"d\"" (x, y) = ((x + 1) `mod` xdim, y)
move _ _ x = x

toFrame :: (Int, Int) -> (Int, Int) -> ListFrame
toFrame (xdim, ydim) (x', y') = ListFrame $ map (\y -> map (\x -> if x == x' && (y == y' || y == y'+1 || y== y'-1) then Pixel 0xff 0xff 0xff else Pixel 0 0 0) [0 .. xdim - 1]) [0 .. ydim - 1]

eventTest :: [Event String] -> (Int, Int) -> (ListFrame, (Int, Int))
eventTest events pixel = (toFrame dim pixel', pixel')
  where pixel' = foldl (\acc (Event mod ev) -> if mod == "KEYBOARD" then move dim ("\"s\"") acc else acc) pixel events
